...
menu *prepare_load_menu_info(menu *menu) {
  ...
  menu->tab_count = 1;
  menu->tabs = (menu_tab *)hhk::Mem_Malloc(sizeof(menu_tab));
...
menu *prepare_menu_info(menu *menu, gb_s *gb) {
  ...
  menu->selected_item = 0;
  menu->tab_count = 4;
  menu->tabs = (menu_tab *)hhk::Mem_Malloc(menu->tab_count * sizeof(menu_tab));
...
void cleanup_menu_info(menu *menu) {
  ...
  for (uint8_t i = 0; i < menu->tab_count; i++) {
    hhk::Mem_Free(menu->tabs[i].items);
  }

  // Free the tabs array
  hhk::Mem_Free(menu->tabs);
}
...
uint8_t emulation_menu(struct gb_s *gb, bool preview_only) {
  ...
  uint16_t *gb_frame_backup =
      (uint16_t *)hhk::Mem_Malloc(LCD_HEIGHT * LCD_WIDTH * sizeof(uint16_t));
